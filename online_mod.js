      ...
      this.search = function (_object, kinopoisk_id, data) {
        ...
        var display = function display(links, have_more, query) {
          if (links && links.length && links.forEach) {
            ...
            // автоматически догружаем полный поиск Rezka и ищем точное название+год там.
            if (!exact.length && search_year && select_title && !_this.deep_searched && typeof query_more === 'function') {
              _this.deep_searched = true;
              return query_more(component.cleanTitle(select_title), 1, [], function (deep_items) {
                ...
                if (!d_year.length) d_year = d_exact.filter(function (c) {
                  return c.year && c.year > search_year - 2 && c.year < search_year + 2;
                });
