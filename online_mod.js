...
(function () {
    ...
    function rezka2Mirror() {
      // MODIFIED: forced local proxy to bypass Anubis anti-bot on rezka.ag
      return 'http://192.168.0.41:3000/rezka';
    }
    ...
    function rezka2(component, _object) {
      ...
      var host = 'http://192.168.0.41:3000/rezka'; // MODIFIED: forced local proxy
      var ref = host + '/';
      ...
      this.search = function (_object, kinopoisk_id, data) {
        ...
        var query_more = function query_more(query, page, data, callback) {
          ...
          network["native"](component.proxyLink(url, prox, prox_enc, prox_enc, 'enc2t'), function (str) {
            str = (str || '').replace(/\n/g, '');
            checkErrorForm(str);
            // MODIFIED: более терпимый разбор — допускаем атрибуты у <div> и вложенную разметку внутри info
            var links = str.match(/<div class="b-content__inline_item-link">\s*<a\s[^>]*>[^<]*<\/a>\s*<div[^>]*>[\s\S]*?<\/div>/g);
            var have_more = !!str.match(/<span class="b-navigation__next\b/);
        ...
        var display = function display(links, have_more, query) {
          if (links && links.length && links.forEach) {
            ...
            // MODIFIED: точное совпадение названия имеет приоритет над "содержит"
            var match_titles = orig_titles.concat(select_title ? [select_title] : []);

            var exact = items.filter(function (c) {
              return component.equalAnyTitle([c.title, c.orig_title], match_titles);
            });

            var exact_year = exact;

            if (exact.length > 1 && search_year) {
              var ey = exact.filter(function (c) {
                return c.year == search_year;
              });
              if (!ey.length) ey = exact.filter(function (c) {
                return c.year && c.year > search_year - 2 && c.year < search_year + 2;
              });
              if (ey.length) exact_year = ey;
            }

            // ровно одно точное совпадение — открываем сразу, без списка
            if (exact_year.length == 1) return getPage(exact_year[0].link);

            // MODIFIED: показ списка вынесен в функцию (нужно для автодогрузки полного поиска)
            var finishDisplay = function finishDisplay() {
              // сортируем список: точные совпадения и близкий год — вверх
              var scoreItem = function (c) {
                var s = 0;
                if (component.equalAnyTitle([c.title, c.orig_title], match_titles)) s += 1000;else if (select_title && component.containsAnyTitle([c.title, c.orig_title], [select_title])) s += 100;
                if (search_year && c.year) s += c.year == search_year ? 500 : Math.max(0, 100 - Math.abs(c.year - search_year));
                return s;
              };

              items = items.map(function (c, i) {
                return {
                  c: c,
                  i: i,
                  s: scoreItem(c)
              ...
              if (cards.length == 1 && is_sure) {
                ...
                if (is_sure) {
                  is_sure = false;
                  if (orig_titles.length) is_sure |= component.equalAnyTitle([cards[0].orig_title, cards[0].title], orig_titles);
                  if (select_title) is_sure |= component.equalAnyTitle([cards[0].title, cards[0].orig_title], [select_title]);
                }
              }
            ...
            // __DEEP_SEARCH_HOOK__
            // Точного совпадения нет в быстром поиске, но год известен из карточки —
            // автоматически догружаем полный поиск Rezka (несколько страниц) и ищем точное название+год там.
            var DIAG = true; // временная диагностика на экране ТВ; убрать после отладки
            var noty = function (m) { try { if (DIAG) Lampa.Noty.show('[dbg] ' + m); } catch (e) {} };
            if (!exact.length) noty('быстрый: точных нет; год=' + search_year + ' назв=' + (select_title || '—') + ' карточек=' + items.length);
            if (!exact.length && search_year && select_title && !_this.deep_searched && typeof query_more === 'function') {
              _this.deep_searched = true;
              var deep_query = component.cleanTitle(select_title);
              noty('старт полного поиска «' + deep_query + '»');

              var scanDeep = function scanDeep(page) {
                query_more(deep_query, page, [], function (deep_items, have_more_deep) {
                  var d_exact = (deep_items || []).filter(function (c) {
                    return component.equalAnyTitle([c.title, c.orig_title], match_titles);
                  });
                  noty('стр.' + page + ': всего ' + (deep_items ? deep_items.length : 0) + ', точных ' + d_exact.length + (have_more_deep ? ', есть ещё' : ''));
                  var d_year = d_exact.filter(function (c) {
                    return c.year == search_year;
                  });
                  if (d_year.length) return getPage(d_year[0].link);

                  var d_near = d_exact.filter(function (c) {
                    return c.year && c.year > search_year - 2 && c.year < search_year + 2;
                  });
                  if (d_near.length) return getPage(d_near[0].link);

                  // на этой странице точного нет — смотрим следующую (до 3 страниц)
                  if (have_more_deep && page < 3) return scanDeep(page + 1);

                  // не нашли за 3 страницы — точное совпадение без учёта года, иначе обычный список
                  if (d_exact.length) return getPage(d_exact[0].link);
                  noty('точное не найдено в полном поиске — показываю список');
                  finishDisplay();
                });
              };
    ...
    function startPlugin() {
      ...
      initFilmix();
      initSettings();
      // ВРЕМЕННО: метка сборки — покажется через ~6с после запуска, если ТВ реально загрузил свежий плагин
      setTimeout(function () { try { Lampa.Noty.show('online_mod: сборка DEEP-4'); } catch (e) {} }, 6000);
    }
